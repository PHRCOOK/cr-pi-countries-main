...
const postActivity = async (req,res) =>{
  try {  
    ...
    if(!name || !difficulty || !season)return res.status(400).json({error: "Faltan datos"})
    
    //responde con un estatus 400 y envia un mensaje si no llega por lo menos un pais dentro del arreglo body.countries
    if(countries.length < 1)return res.status(400).json({error: "Se necesita un pais"})
    ...
    for (let country of countries){
        const regex = /^[A-Z]{3}$/
        const countryValid = await (regex.test(country.toUpperCase()) && getCountryById(country.toUpperCase()))
        if(!countryValid) countriFail.push(country)
    }

    //responde con un estatus 400 y un mensaje si alguno de los codigos no es valido
    if(countriFail.length > 0) return res.status(400).json({error:`Los siguiemtes codigos de pais no son validos [${countriFail.join(" ")}]`})
    ...
    //llama al acontroller setActivity con todos los datos para crear la actividad y los paises para relacionar
