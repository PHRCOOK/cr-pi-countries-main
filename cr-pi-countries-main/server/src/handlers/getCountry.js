...
const getCountry = async (req,res)=> {
    try{
        ...
        const regex = /^[A-Z]{3}$/ //se verifica si son 3 caracteres alfaveticos de la A la Z
       
        //si no cumple con el formato CCA3 responde con un estatus 400 y un json com un mensaje  
        if(!regex.test(id)) return res.status(400).json({error:`Id: <${id}> invalido!, se necesita un ISO Alpha 3 country code (cca3)`})
        ...
        if(!country) return res.status(404).json({error:`No se encuentra un pais con el Id: ${id}`})
